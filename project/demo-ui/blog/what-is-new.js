...
> 1. **Standalone:** Removed dependency to appc and introduced a new CLI (@axway/api-builder)

> 2. **Plugin architecture:** Introduced new plugin architecture leveraging npm modules:
...
> 3. **Custom flow-nodes:** plugins with input parameters and autocomplete
* Added new asynchronous HTTP rest client plugin to the flow
* Added new asynchronous Swagger plugin to the flow

> 4. **Logging:** Revised logging format to stream to stdout
> 5. **Configuration:** Revised configuration to have only \`default\` and \`local\` sets of configuration files
...
![REST flow-node](static/images/rest-client.png)
...
![Swagger flow-node](static/images/swagger-flow-node.png)
